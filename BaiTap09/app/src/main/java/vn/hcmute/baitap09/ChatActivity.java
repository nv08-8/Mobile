...
/**
 * ChatActivity - Activity cho khách hàng chat với manager
 *
 * Chức năng:
...
public class ChatActivity extends AppCompatActivity implements SocketManager.SocketListener {
    ...
    private void initViews() {
        ...
        chatTitle.setText("Customer Support");
    }

    private void setupUser() {
        // Tạo user ID duy nhất cho khách hàng
        // Trong thực tế, bạn có thể lấy từ SharedPreferences hoặc database
        String userId = "customer_" + UUID.randomUUID().toString().substring(0, 8);
        String userName = "Customer " + userId.substring(9);

        currentUser = new User(userId, userName, "customer");
    }

    private void setupRecyclerView() {
        chatAdapter = new ChatAdapter(currentUser.getUserId());
        LinearLayoutManager layoutManager = new LinearLayoutManager(this);
        layoutManager.setStackFromEnd(true); // Scroll to bottom when new message arrives
    ...
    public void onNewMessage(ChatMessage message) {
        runOnUiThread(() -> {
            // Don't add our own messages again (already added when sending)
    ...
    public void onTyping(String userId, String userName) {
        runOnUiThread(() -> {
            // Only show if it's not the current user typing
    ...
    public void onUserJoined(User user) {
        runOnUiThread(() -> {
            Toast.makeText(this, user.getUserName() + " joined", Toast.LENGTH_SHORT).show();
        });
    }
    ...
    protected void onDestroy() {
        ...
        // Don't disconnect here if you want to keep connection alive
